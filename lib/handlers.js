...
function createHandlers(server, settings) {
  ...
  function apiComplete(req, reply) {
    _getResponsePage(req, reply, ['complete'])
  }

  function _getResponsePage(req, reply, search) {
    // responses
    ...
    req.querybox.run('find-responses', [search, page], onrows)
    ...
    function sendResponse() {
      ...
      return reply({objects: rows})
        .header('X-Total-Responses', count)
    }
  }

  function apiIssues(req, reply) {
    var page = req.query.p ? 10 * Math.abs(Number(req.query.p)) : 0
    req.querybox.run('find-issue-responses', [page], onrows)
  ...
  function apiRepoIssues(req, reply) {
    var page = req.query.p ? 10 * Math.abs(Number(req.query.p)) : 0
    req.querybox.run('find-issue-repo-responses', [req.params.repoUser, req.params.repoName, page], onrows)
